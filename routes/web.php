...
use App\Http\Controllers\SuperAdmin\StockTransactionController;
...
Route::middleware(['auth', 'waiter'])->prefix('waiter')->name('waiter.')->group(function () {
    ...
    Route::post('tables/{table}/unassign', [WaiterDashboardController::class, 'unassignTable'])->name('tables.unassign');
    Route::get('orders/create/{table}', [WaiterOrderController::class, 'create'])->name('orders.create');
    ...
    Route::patch('orders/{order}/cancel', [WaiterOrderController::class, 'cancelOrder'])->name('orders.cancel');
});
