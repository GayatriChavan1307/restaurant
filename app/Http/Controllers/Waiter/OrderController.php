...
class OrderController extends Controller
{
    ...
    public function updateItem(Request $request, Order $order, OrderItem $orderItem)
    {
        ...
        // KitchenPrint for modification (details depend on how kitchen handles this)
        // For simplicity, a general "order modified" might be enough, or specific item change.
        // This assumes the whole order or new items will be re-printed.
        // A more advanced system would track individual item changes for kitchen print.
    ...
    public function removeItem(Order $order, OrderItem $orderItem) // This is logical cancel
    {
        ...
        $order->calculateTotal(); // Recalculate total
   ...
   public function cancelOrder(Request $request, Order $order)
    {
        if ($order->status === 'paid' || $order->status === 'cancelled') {
            return back()->with('error', 'Order is already ' . $order->status . ' and cannot be cancelled.');
        }
        ...
        if ($receptionUsers->isEmpty()) {
            \Log::warning('No reception users found to notify for order cancellation: Order #' . $order->id);
        }
        foreach ($receptionUsers as $user) {
            Notification::create([
                'user_id' => $user->id,
                'message' => "Order #{$order->id} for Table {$table->name} has been cancelled.",
        ...
        return redirect()->route('waiter.dashboard')->with('success', 'Order cancelled successfully. Table is now available.');
    }
