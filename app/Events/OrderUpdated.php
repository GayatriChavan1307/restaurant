...
class OrderUpdated implements ShouldBroadcast
{
    ...
    public function __construct(Order $order)
    {
        ...
        foreach ($receptionUsers as $user) {
            Notification::create([
                'user_id' => $user->id,
                'message' => "Order #{$order->id} at Table {$order->restaurantTable->name} has been updated.",
    ...
    {
        return [
            'message' => "Order #{$this->order->id} at Table {$this->order->restaurantTable->name} has been updated.",
